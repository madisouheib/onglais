...
if($level == 3 ){
    ...
    $levels = levels::select('level_test.*', 'languages.lang_field')->leftJoin('languages', 'languages.id_lang', '=', 'level_test.id_lang')->withCount('test')->orderBy('id_test', 'desc')->paginate(10);
...
$ListLang = User::leftJoin('learners', 'learners.id_learner', '=', 'users.id')->where('users.id','=',$id)->pluck('id_lang')->toArray();
    ...
    $levels = levels::select('level_test.*', 'languages.lang_field')->leftJoin('languages', 'languages.id_lang', '=', 'level_test.id_lang')->withCount('test')->whereIn('level_test.id_lang',$ListLang)->orderBy('id_test', 'desc')->paginate(10);
...
$Listusers = LevelList::leftJoin('users', 'users.id', '=', 'list_level_test.id_learner')->leftJoin('levels', 'levels.id_level', '=', 'list_level_test.id_level')->where('id_test','=',$id)->paginate(10); 
