...
$LevRes = Reservation::leftJoin('sessions','sessions.id_session','reservation.id_session')->where('id_learner','=',$id)->pluck('sessions.id_lang')->toArray();


if($Group == 3 ){

    $DataSession = Sessions::select('levels.*', 'languages.*','sessions.*')->leftJoin('languages','languages.id_lang','sessions.id_lang')->leftJoin('levels','levels.id_level','sessions.id_level')->withCount('reservation')->paginate(60);
...
$counted = count($Getlang);

 //dd($counted);
if($counted == 2 ){   
    
    $DataSession = Sessions::select('levels.*', 'languages.*','sessions.*')->leftJoin('languages','languages.id_lang','sessions.id_lang')->leftJoin('levels','levels.id_level','sessions.id_level')->has('reservation','<',15)->withCount('reservation')->whereNotIn('sessions.id_lang', $LevRes)->where([['sessions.id_level', '=', $GetLevels['0']],
    ['sessions.id_lang', '=',$Getlang['0']]])->orWhere([['sessions.id_level', '=', $GetLevels['1']],
    ['sessions.id_lang', '=',$Getlang['1']]])->paginate(30);
    //dd($Getlang['0']);
}else {  
    
    $DataSession = Sessions::select('levels.*', 'languages.*','sessions.*')->leftJoin('languages','languages.id_lang','sessions.id_lang')->leftJoin('levels','levels.id_level','sessions.id_level')->has('reservation','<',15)->withCount('reservation')->whereIn('sessions.id_level',$GetLevels)->whereIn('sessions.id_lang',$Getlang)->paginate(10);
...
$DataParticipants=  Reservation::leftJoin('users','users.id','reservation.id_learner')->where('id_session','=',$id)->paginate(40);
...
public function update_link(Request $request){
    ...
    $idsession = request('id') ; 
     $zoomlink = request('linkzoom') ; 
$GetSession = Sessions::find($idsession);
    
    $GetSession->zoom_link = $zoomlink;
$GetSession->save();
...
$CountReserv = Reservation::where('id_session','=',$idtest)->where('id_learner','=',$idlearner)->count();
if($CountReserv == 0 ){
