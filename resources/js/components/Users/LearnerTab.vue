...
<button class=" btn btn-sm btn-glow-info btn-info">  <i style="margin-right: -2px;" data-toggle="modal" data-target="#ModalEditPass"  @click="getUser(user.id)"   class="fas fa-edit"></i> </button>
...
GetUsers(page = 1,idlang)
 {
     ...
     axios.get('/api/getlearners/?page='+page)
     .then(response =>
     { 
...
this.GetUsers(page);
 ...
 GetCountryUsers(page = 1 )
 {
     ...
     axios.get('/api/getcountrylearners/'+this.countrys+'/?page='+page)
     .then(response =>
     { 
 ...
 GetFiltreUsers(page=1,idlang)
 {
     ...
     axios.get('/api/getfiltreusers/'+this.idlang+'/?page='+page)
     .then(response =>
     { 
           this.users= response.data 
